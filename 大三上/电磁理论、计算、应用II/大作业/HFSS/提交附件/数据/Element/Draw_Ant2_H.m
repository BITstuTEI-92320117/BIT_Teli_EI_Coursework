...
% 用于从以下文本文件中导入数据的脚本:
%
%    filename: D:\lessons_and_papers\专业课\电磁理论、计算、应用\电磁理论、计算、应用II\论文复现\数据\Ant1\HGain Plot_2.csv
%
% 由 MATLAB 于 2025-11-08 20:28:34 自动生成
...
% 导入数据
HGainPlot2 = readtable("D:\lessons_and_papers\专业课\电磁理论、计算、应用\电磁理论、计算、应用II\论文复现\数据\Ant1\HGain Plot_2.csv", opts);

%% 转换为输出类型
HGainPlot2 = table2array(HGainPlot2);

% 提取角度和增益
theta = HGainPlot2(:,1);    % 读取原始角度数据（范围：-180°~180°）
theta = (theta + 180)*2*pi/360;  % 将角度转换为弧度，并映射到0~2π范围
gain = HGainPlot2(:,2);     % 读取增益数据（单位：dBi）
gc = gain(1:180);
...
polarplot(pi/2 - theta, gain, 'r-', 'LineWidth', 1.2, 'DisplayName', 'Ant. 2');  % 绘制极坐标曲线，红色加粗实线，图例显示“Meas.”
rlim([-8,8])  % 设置径向（增益）显示范围
...
% 调整径向刻度标签样式（匹配增益范围）
ax.RTick = [-4.8 -1.6 1.6 4.8];  % 设置径向刻度位置
ax.RTickLabels = {'-4.8dBi', '-1.6dBi', '1.6dBi', '4.8dBi'};  % 自定义径向刻度标签（带单位）
ax.FontName = 'Times New Roman';  % 统一设置字体为Times New Roman
ax.FontWeight = 'bold';  % 字体加粗

print('Ant2_H', '-dpng', '-r300');  % 保存图形为300dpi的PNG格式，文件名为"Ant2_H”