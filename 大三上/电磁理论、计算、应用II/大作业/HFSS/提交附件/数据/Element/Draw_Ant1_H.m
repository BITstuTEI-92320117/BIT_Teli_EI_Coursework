...
% 用于从以下文本文件中导入数据的脚本:
%
%    filename: D:\lessons_and_papers\专业课\电磁理论、计算、应用\电磁理论、计算、应用II\论文复现\数据\Ant1\HGain Plot 1.csv
%
% 由 MATLAB 于 2025-11-08 20:08:14 自动生成
...
% 导入数据
HGainPlot1 = readtable("D:\lessons_and_papers\专业课\电磁理论、计算、应用\电磁理论、计算、应用II\论文复现\数据\Ant1\HGain Plot_1.csv", opts);

%% 转换为输出类型
HGainPlot1 = table2array(HGainPlot1);

% 提取角度和增益
theta = HGainPlot1(:,1);    % 读取原始角度数据（范围：-180°~180°）
theta = (theta + 180)*2*pi/360;  % 将角度转换为弧度，并映射到0~2π范围
gain = HGainPlot1(:,2);     % 读取增益数据（单位：dBi）
gc = gain(1:180);
...
% 调整径向刻度标签样式（匹配增益范围）
ax.RTick = [-10 -5 0 5];  % 设置径向刻度位置
ax.RTickLabels = {'-10dBi', '-5dBi', '0dBi', '5dBi'};  % 自定义径向刻度标签（带单位）
ax.FontName = 'Times New Roman';  % 统一设置字体为Times New Roman
ax.FontWeight = 'bold';  % 字体加粗

print('Ant1_H', '-dpng', '-r300');  % 保存图形为300dpi的PNG格式，文件名为“Ant1_H”