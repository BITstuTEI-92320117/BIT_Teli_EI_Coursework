...
% 用于从以下文本文件中导入数据的脚本:
%
%    filename: D:\lessons_and_papers\专业课\电磁理论、计算、应用\电磁理论、计算、应用II\论文复现\数据\H_Array\H_Plane_E4.csv
%
% 由 MATLAB 于 2025-11-12 08:44:19 自动生成
...
% 指定列名称和类型
opts.VariableNames = ["Var1", "Var2", "Var3", "Thetadeg", "dBGainTotal"];
...
% 导入数据
HPlaneE4 = readtable("D:\lessons_and_papers\专业课\电磁理论、计算、应用\电磁理论、计算、应用II\论文复现\数据\H_Array\H_Plane_E4.csv", opts);

%% 转换为输出类型
HPlaneE4 = table2array(HPlaneE4);

% 提取角度和增益
theta = HPlaneE4(:,1);    % 读取原始角度数据（范围：-180°~180°）
theta = (theta + 180)*2*pi/360;  % 将角度转换为弧度，并映射到0~2π范围
gain = HPlaneE4(:,2);     % 读取增益数据（单位：dBi）
gc = gain(1:180);
...
print('H_Plane_E4', '-dpng', '-r300');  % 保存图形为300dpi的PNG格式，文件名为“Ant1_H”