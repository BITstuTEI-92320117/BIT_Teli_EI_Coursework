...
% 用于从以下文本文件中导入数据的脚本:
%
%    filename: D:\lessons_and_papers\专业课\电磁理论、计算、应用\电磁理论、计算、应用II\论文复现\数据\Ant1\EGain Plot_1.csv
%
% 由 MATLAB 于 2025-11-08 20:16:01 自动生成
...
% 导入数据
EGainPlot1 = readtable("D:\lessons_and_papers\专业课\电磁理论、计算、应用\电磁理论、计算、应用II\论文复现\数据\Ant1\EGain Plot_1.csv", opts);

%% 转换为输出类型
EGainPlot1 = table2array(EGainPlot1);

% 提取角度和增益
theta = EGainPlot1(:,1);    % 角度数据（-180°~180°）
theta = (theta + 180)*2*pi/360;
gain = EGainPlot1(:,2);     % 增益数据（dBi）
gc = gain(1:180);
gain(1:181) = gain(181:361);
gain(182:361) = gc;

% 调整曲线样式（单曲线，加粗实线）
figure('Color','white');    % 创建白色背景的图形窗口
polarplot(pi/2 - theta, gain, 'r-', 'LineWidth', 1.2, 'DisplayName', 'Ant. 1');  % 绘制极坐标曲线，红色加粗实线，图例显示“Meas.”
rlim([-25,10])  % 设置径向（增益）显示范围
...
% 调整径向刻度标签样式（匹配增益范围）
ax.RTick = [-18 -11 -4 3];  % 设置径向刻度位置
ax.RTickLabels = {'-18dBi', '-11dBi', '-4dBi', '3dBi'};  % 自定义径向刻度标签（带单位）
ax.FontName = 'Times New Roman';  % 统一设置字体为Times New Roman
ax.FontWeight = 'bold';  % 字体加粗

print('Ant1_E', '-dpng', '-r300');  % 保存图形为300dpi的PNG格式，文件名为“Ant1_H”